...
- `zoom: [-0.25, -0.5]`
    - ピクチャを1フレームごとに横に `-0.25` ピクセル、縦に `-0.5` ピクセル拡大縮小しますよーと示す文です
- `anchor: 8`
    - ピクチャの座標や拡大縮小の起点を上中央 (テンキーで `8` は上にあたる) にしますよーと示す文です
...
という動きとなっています。 `next` プロパティと `map` プロパティは扱いは難しいですが、組み合わせることでより複雑かつ魅力的な動きを作ることができるので、腕のある方は是非チャレンジしてみてください。
